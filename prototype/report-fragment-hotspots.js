...
function hasCoreSlots(assertion) {
  const slots = (assertion && assertion.slots) || {};
  return (
    Array.isArray(slots.actor) && slots.actor.length > 0 ||
    Array.isArray(slots.theme) && slots.theme.length > 0 ||
    Array.isArray(slots.attr) && slots.attr.length > 0 ||
    Array.isArray(slots.topic) && slots.topic.length > 0 ||
    Array.isArray(slots.location) && slots.location.length > 0
  );
}
...
function buildSeedReport(doc, seedId) {
  ...
  const clauseKeyByAssertionId = new Map(
    assertions.map((a) => [String(a.id || ''), assertionClauseWindowKey(a, segTokensById)])
  );
  ...
  const segments = Array.from(bySegment.keys()).sort((a, b) => a.localeCompare(b)).map((segmentId) => {
    ...
    const carriers = fragmentAssertions
      ...
      .map((source) => {
        ...
        const hosts = segAssertions
          ...
          .map((h) => ({
            id: String(h.id || ''),
            i: tokenIndex.get(String((((h || {}).predicate) || {}).head_token_id || '')),
            host: h,
          }))
          .sort((a, b) => {
        ...
        const segmentHosts = segAssertions
          .filter((h) => h && h.id !== source.id && isLexicalHost(h))
          .map((h) => ({
            id: String(h.id || ''),
            i: tokenIndex.get(String((((h || {}).predicate) || {}).head_token_id || '')),
            host: h,
        ...
        const containmentPass = hasClauseHost ? clauseContainmentPass : segmentContainmentPass;
        const effectiveHostExists = hasClauseHost || hasSegmentHost;
        let failureReason = null;
        if (hasCoreSlots(source)) failureReason = 'has_core_slots';
        else if (!effectiveHostExists) failureReason = 'no_host';
        else if (!containmentPass) failureReason = 'no_containment';
        return {
          assertion_id: String(source.id || ''),
          predicate_class: sourceClass,
          has_clause_local_lexical_host: hasClauseHost,
          evidence_containment_pass: containmentPass,
