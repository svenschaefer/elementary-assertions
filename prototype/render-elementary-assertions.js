// Usage:
// node render-elementary-assertions.js --in <path> --format <txt|md> --segments <true|false> --mentions <true|false> --coverage <true|false> --debug-ids <true|false> [--layout <compact|readable|table|meaning>] [--normalize-determiners <true|false>] [--render-uncovered-delta <true|false>] [--out <path>]
const fs = require('fs');
...
function usage() {
  console.error(
    'Usage: node render-elementary-assertions.js --in <path> [--out <path>] --format <txt|md> --segments <true|false> --mentions <true|false> --coverage <true|false> --debug-ids <true|false> [--layout <compact|readable|table|meaning>] [--normalize-determiners <true|false>] [--render-uncovered-delta <true|false>]'
  );
}
...
function readYaml(filePath) {
  ...
  try {
    ...
    throw new Error(`Cannot read input: ${err && err.message ? err.message : String(err)}`);
  }
  try {
    return YAML.parse(raw);
  } catch (err) {
    throw new Error(`YAML parse error: ${err && err.message ? err.message : String(err)}`);
  }
}
...
(function main() {
  try {
    ...
    const layout = getArg(args, '--layout') || 'compact';
    const segmentsFlag = getArg(args, '--segments');
    const mentionsFlag = getArg(args, '--mentions');
    const coverageFlag = getArg(args, '--coverage');
    const debugIdsFlag = getArg(args, '--debug-ids');
    const normalizeDeterminersFlag = getArg(args, '--normalize-determiners');
    const renderUncoveredDeltaFlag = getArg(args, '--render-uncovered-delta');

    if (!inPath || !format || segmentsFlag === null || mentionsFlag === null || coverageFlag === null || debugIdsFlag === null) {
      usage();
      process.exit(2);
    }
    ...
    const options = {
      format,
      layout,
      segments: parseBool('--segments', segmentsFlag),
      mentions: parseBool('--mentions', mentionsFlag),
      coverage: parseBool('--coverage', coverageFlag),
      debugIds: parseBool('--debug-ids', debugIdsFlag),
      normalizeDeterminers: normalizeDeterminersFlag === null ? true : parseBool('--normalize-determiners', normalizeDeterminersFlag),
      renderUncoveredDelta: renderUncoveredDeltaFlag === null ? false : parseBool('--render-uncovered-delta', renderUncoveredDeltaFlag),
    };

    const data = readYaml(path.resolve(inPath));
    const refs = buildIntegrity(data);
    const rendered = render(data, options, refs);
