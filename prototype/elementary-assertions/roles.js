...
function collectAssertionMentionRefs(assertion) {
  ...
  if (out.size === 0 && assertion && assertion.slots && typeof assertion.slots === 'object') {
    const slots = assertion.slots;
    for (const slotName of ['actor', 'theme', 'attr', 'topic', 'location']) {
      for (const mentionId of slots[slotName] || []) out.add(mentionId);
    }
